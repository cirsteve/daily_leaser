  ...
  render() {
    ...
    return (
      <div className="inline-divs">
        <div>
            <Link to={`/blocklease/${this.props.contractAddr}`}><h2 className="title">BlockSpace</h2></Link>
        </div>
        <div className="navbar navbar-right">
            {hrefs.map(l => <Link key={l.label} to={`/blocklease/${this.props.contractAddr}/${l.href}`}>{l.label}</Link>)}
        </div>
      </div>
    )
  }
